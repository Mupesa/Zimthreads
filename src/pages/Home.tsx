  ...
  return (
    <main>
      ...
      <section className="relative min-h-[90vh] flex items-center overflow-hidden bg-[#f5f2ec]">
        ...
        {/* Trust badges */}
        <div className="absolute bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xs border-t border-[#e5e1d8]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex flex-wrap justify-center sm:justify-between gap-3 sm:gap-6">
            {badges.map((b) => {
              ...
              return (
                ...
                >
                  <IconComp className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#4a5c2d]" />
                  <span className="text-[9px] sm:text-[10px] font-bold tracking-widest uppercase text-[#1a1a1a]">
                    {b.label}
                  </span>
                </div>
              )
      ...
      {/* Category bar */}
      <section className="bg-[#1a1a1a] py-3.5 overflow-x-auto scrollbar-hide">
        <div className="flex gap-6 sm:gap-8 px-4 sm:px-6 min-w-max mx-auto justify-start sm:justify-center">
          {categories.map((c) => (
            <Link
              key={c}
              to="/services"
              className="text-[#9ca3af] hover:text-[#86a84e] transition-colors whitespace-nowrap text-[10px] sm:text-[11px] font-semibold tracking-widest uppercase"
            >
              {c}
            </Link>
          ))}
            ...
            >
              ...
              <div>
                ...
                <div className="p-5 sm:p-6">
                  ...
                  {s.tagline && (
                    <p className="text-[11px] font-bold uppercase tracking-wider text-[#4a5c2d] mb-4">
                      "{s.tagline}"
                    </p>
                  )}
                  <button
                    onClick={() => handleBookService(s.id)}
                    className="w-full py-3 bg-[#f5f2ec] text-[#1a1a1a] text-[11px] font-bold tracking-widest uppercase hover:bg-[#4a5c2d] hover:text-[#f5f2ec] transition-colors text-center block mt-2"
                  >
                    BOOK {s.title} →
                  </button>
                </div>
            ...
            {dealServices.map((deal) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-[#86a84e] font-semibold mb-3 uppercase tracking-wider">
                    {deal.tagline}
                  </p>
                  <p className="font-display text-4xl font-extrabold text-white mb-6">
                    {deal.price}
                  </p>
                </div>
                <button
      ...
      <section className="py-20 max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-6">
          {[
            ...
            <div
              key={st.step}
              className="p-4 sm:p-6 bg-white border border-[#e5e1d8] flex flex-col justify-between hover:border-[#4a5c2d] transition-colors"
            >
              <div>
                <span className="font-display text-sm font-black text-[#4a5c2d] tracking-widest block mb-1.5">
                  {st.step}
                </span>
